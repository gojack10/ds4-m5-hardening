...
typedef struct {
    ...
    char **prop;
    /* Retain GLM/DeepSeek type validation and Qwen's full-schema boundary. */
    char **prop_type;
    char **prop_schema;
    int len;
    int cap;
...
static void tool_schema_order_free(tool_schema_order *o) {
    ...
    for (int i = 0; i < o->len; i++) {
        ...
        if (o->prop_schema) free(o->prop_schema[i]);
    }
    free(o->prop);
    free(o->prop_type);
    free(o->prop_schema);
        ...
        if (!strcmp(key, "properties")) {
            ...
            while (*p && *p != '}') {
                ...
                tool_schema_order_prop_push(order, prop,
                                             parse_schema_property_type(schema), schema);
                json_ws(&p);
                if (*p == ',') p++;
                json_ws(&p);
            }
 ...
 * tool result as structured JSON. Keep the fork's OpenAI fast path for GLM;
 * other model families retain upstream's Anthropic-only path. */
static void message_prepare_live_continuation(server *s, request *r,
                                              const chat_msgs *msgs) {
    if (!r || (r->api != API_ANTHROPIC &&
               !(r->api == API_OPENAI && r->model_syntax == SERVER_MODEL_SYNTAX_GLM)) ||
...
static bool parse_chat_request(ds4_engine *e, server *s, const char *body, int def_tokens,
    ...
    message_prepare_live_continuation(s, r, &msgs);
...
static bool dsml_parse_leaf_param_json(const char **p_in, const char *param_start,
    ...
    /* Nested properties do not use a top-level schema type. */
...
static bool parse_glm_generated_message_ex(const char *text,
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            const char *schema_type =
                tool_schema_orders_find_prop_type(orders, name, key);
            tool_call_json_args_add(&args, name, key, value, "true",
                                    schema_type, diag);
            free(key);
            free(raw_value);
            free(value);
...
static bool parse_qwen_generated_message_ex(const char *text,
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            tool_call_json_args_add(&args, NULL, key, value,
                                    is_string ? "true" : "false", NULL, NULL);
            free(key);
            free(value);
            p = value_end + strlen(param_end);
        }
...
static bool parse_generated_message_ex_for_syntax_with_schema(
        server_model_syntax syntax,
        const char *text,
        bool require_thinking_closed,
        const tool_schema_orders *orders,
...
static bool parse_generated_message_ex_for_syntax(
        server_model_syntax syntax,
        const char *text,
        bool require_thinking_closed,
        char **content_out,
        char **reasoning_out,
        tool_calls *calls) {
    return parse_generated_message_ex_for_syntax_with_schema(
        syntax, text, require_thinking_closed, NULL, NULL,
...
static DS4_SERVER_MAYBE_UNUSED bool parse_generated_message_ex(
        ...
        char **reasoning_out,
        tool_calls *calls) {
    return parse_generated_message_ex_for_syntax(
        SERVER_MODEL_SYNTAX_DEEPSEEK, text, require_thinking_closed,
        content_out, reasoning_out, calls);
}
...
static bool parse_generated_message_for_response_for_syntax_with_schema(
        server_model_syntax syntax,
        const char *text,
        bool has_tools,
        bool saw_tool_start,
    ...
    bool parsed_ok = parse_generated_message_ex_for_syntax_with_schema(
        syntax, text ? text : "", require_thinking_closed, orders, &diag,
        content_out, reasoning_out, calls);
    ...
    /* Schema mismatch the parser could not coerce: refuse the call instead of
     * forwarding invalid arguments, and give the model a correction message. */
...
static bool parse_generated_message_for_response_for_syntax(
        ...
        bool *recovered_out,
        const tool_schema_orders *orders) {
    return parse_generated_message_for_response_for_syntax_with_schema(
        syntax, text, has_tools, saw_tool_start, require_thinking_closed, orders,
        finish_io, err, errlen, content_out, reasoning_out, calls, recovered_out,
        NULL, NULL);
}
...
static bool should_remember_thinking_checkpoint(const request *r,
    ...
    if (r->model_syntax != SERVER_MODEL_SYNTAX_QWEN) {
        ...
        const bool qwen_chat = r->api != API_RESPONSES && r->api != API_ANTHROPIC;
...
static char *build_chat_checkpoint_visible_text(const request *r,
    ...
    if (!r || !r->prompt_text) return NULL;
    if (!r->has_tools)
        return build_thinking_visible_text(r, content);
...
static void remember_thinking_checkpoint(server *s, server_slot *slot,
    ...
    char *visible = j->req.model_syntax == SERVER_MODEL_SYNTAX_QWEN ?
        build_thinking_visible_text(&j->req, content) :
        build_chat_checkpoint_visible_text(&j->req, content, reasoning);
    if (!visible) return;
...
decode_again:
        ...
        strcmp(finish, "length") != 0 && !client_stop)
    {
        ...
        if (try_repair_dsml(text.ptr, text.len, &repaired)) {
            ...
            bool repair_ok = parse_generated_message_ex_for_syntax_with_schema(
                j->req.model_syntax, repaired.ptr, false,
                &j->req.tool_orders, NULL,
    ...
    if (j->req.kind == REQ_CHAT) {
        ...
        bool parsed_ok = parse_generated_message_for_response_for_syntax_with_schema(
            j->req.model_syntax,
            text.ptr ? text.ptr : "",
            j->req.has_tools,
            saw_tool_start,
...
/* The fork's GLM integer coercion/rejection must not change Qwen's upstream
 * boundary: non-JSON parameter values remain strings, even for integer schemas. */
static void test_qwen_does_not_apply_glm_argument_repair(void) {
    tool_schema_orders orders = {0};
    tool_schema_orders_add_json(&orders,
        "{\"name\":\"count\",\"parameters\":{\"type\":\"object\",\"properties\":{"
        "\"n\":{\"type\":\"integer\"}}}}");
    const char *values[] = {"3 units", "not an integer"};
    for (size_t i = 0; i < sizeof(values) / sizeof(*values); i++) {
        buf raw = {0};
        buf_printf(&raw, "<tool_call><function=count><parameter=n>%s"
                        "</parameter></function></tool_call>", values[i]);
        tool_calls calls = {0};
        char *content = NULL, *reasoning = NULL, *coerced = NULL, *rejected = NULL;
        const char *finish = "stop";
        ...
        TEST_ASSERT(parse_generated_message_for_response_for_syntax_with_schema(
            SERVER_MODEL_SYNTAX_QWEN, raw.ptr, true, true, false, &orders,
            &finish, err, sizeof(err), &content, &reasoning, &calls, &recovered,
            &coerced, &rejected));
        TEST_ASSERT(!coerced && !rejected && !recovered && calls.len == 1);
        if (calls.len == 1) {
            json_args args = {0};
            TEST_ASSERT(json_args_parse(calls.v[0].arguments, &args));
            TEST_ASSERT(args.len == 1 && args.v[0].is_string &&
                        !strcmp(args.v[0].value, values[i]));
            json_args_free(&args);
        }
...
static void test_anthropic_live_tail_renders_tool_results_only(void) {
    ...
    message_prepare_live_continuation(NULL, &r, &msgs);
...
static void test_usage_metrics_persist(void) {
    char tmpl[] = "/tmp/ds4-usage-test.XXXXXX";
    char *dir = mkdtemp(tmpl);
    TEST_ASSERT(dir != NULL);
    if (!dir) return;
    ...
    server s = {0};
    pthread_mutex_init(&s.usage_mu, NULL);
    s.usage_dir = xstrdup(dir);
    s.usage_log_path = path_join(dir, "usage.jsonl");
    s.usage_stats_path = path_join(dir, "stats.json");
    request r;
    request_init(&r, REQ_CHAT, 64);
    r.model = xstrdup("deepseek-v4-flash");
    r.cache_read_tokens = 30;
    r.cache_write_tokens = 70;
    usage_record_request_complete(&s, &r, 100, 12, "stop", 1.5, 2.5);

    FILE *fp = fopen(s.usage_stats_path, "r");
    TEST_ASSERT(fp != NULL);
    char stats[2048] = {0};
    if (fp) {
        fread(stats, 1, sizeof(stats) - 1, fp);
        fclose(fp);
    }
    TEST_ASSERT(strstr(stats, "\"total_prompt_tokens\": 100") != NULL);
    TEST_ASSERT(strstr(stats, "\"total_completion_tokens\": 12") != NULL);
    TEST_ASSERT(strstr(stats, "\"total_cached_tokens\": 30") != NULL);
    TEST_ASSERT(strstr(stats, "\"total_cache_write_tokens\": 70") != NULL);

    char *log_path = xstrdup(s.usage_log_path);
    char *stats_path = xstrdup(s.usage_stats_path);
    request_free(&r);
    usage_close(&s);
    pthread_mutex_destroy(&s.usage_mu);
    unlink(log_path);
...
static void test_deepseek41_live_result_order(void) {
    ...
    for (int anthropic = 0; anthropic < 2; anthropic++) {
        ...
        if (anthropic) message_prepare_live_continuation(&s, &r, &msgs);
...
static void ds4_server_unit_tests_run(void) {
    ...
    test_qwen_does_not_apply_glm_argument_repair();
    ...
    test_dsml_schema_types_are_coerced_or_rejected();
